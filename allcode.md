...
Generated: 2026-02-21T21:23:37

## `.dev.vars.example`
```text
SILICONFLOW_API_KEY=your_siliconflow_api_key
SILICONFLOW_MODEL=deepseek-ai/DeepSeek-V3.2
SILICONFLOW_GM_MODEL=Qwen/Qwen3-8B
...
async function runGameToEnd() {
  ...
  setBusy(true);
  try {
    const maxSteps = Number(ui.maxStepsInput.value) || 32;
    if (!currentState) {
      const result = await requestJson("/api/game/new");
      currentState = result.state;
      renderAll();
  ...
  <body>
    ...
    <main class="app-shell">
      ...
      <section class="controls panel">
        ...
        <label class="steps">
          最大步数
          <input id="max-steps-input" type="number" min="1" max="128" value="32" />
        </label>
      </section>
